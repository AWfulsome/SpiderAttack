...
enum {monster, myHero, opHero};
enum {WAIT, MOVE, SPELL};
enum {WIND, SHIELD, CONTROL};
...
double distance(Entity a, pair<int, int> b)
{
    return hypot(abs(a.x - b.first), abs(a.y - b.second));
}//distance

void set_defender(vector<vector<Entity>> &entities, int defenderID)
{
    Entity defender = entities[myHero][defenderID];
    ...
    for (int i = 0; myMana >= 10 && i < entities[opHero].size(); i++)
    {
        if (distance(defender, entities[opHero][i]) <= 1200)
        {
            action = SPELL;
            spellType = WIND;
            break;
        }//if
        else if (myMana >= 100 && distance(defender, entities[opHero][i]) <= 1500)
        {
            action = SPELL;
            spellType = CONTROL;
    ...
    if (entities[monster].empty()) action = MOVE;
    else if (entities[monster][0].shield_life == 0 && myMana >= 10 && distance(defender, entities[monster][0]) < 1200 && entities[monster][0].distToMyBase < 2000)
    {
        action = SPELL;
        spellType = WIND;
    }//else if
    else if (entities[monster][0].distToMyBase < 5000)
    {
        action = MOVE;
        tie(toX, toY) = tie(entities[monster][0].x, entities[monster][0].y);
    }//else if

    if (defender.shield_life == 0)
    {
        for (auto o : entities[opHero])
        {
...
void set_reaper(vector<vector<Entity>> &entities, int reaperID, int standX, int standY, bool farReaper = false)
{
    ...
    {
        if (entities[monster][i].distToMyBase <= 5000 || (!farReaper && 7000 <= entities[monster][i].distToMyBase)) continue;
    ...
    if (entities[monster].empty()) targetMonsterIndex = -1;
    ...
    if (targetMonsterIndex == -1)
    {
        ...
        if (myBaseX) toX = 17630 - standX, toY = 9000 - standY;
        else toX = standX, toY = standY;
    }//if
    else
    {
        action = MOVE;
    ...
    {
        if (entities[opHero][i].distToMyBase > 8000) continue;
...
void set_attacker(vector<vector<Entity>> &entities, int attackerID)
{
    ...
    if (myBaseX) toX = 7630, toY = 4500;
    else toX = 10000, toY = 4500;
    ...
    if (distance(attacker, make_pair(opBaseX, opBaseY)) < 4000) sentCnt = 0;
    else sentCnt = max(0, sentCnt - 1);
    ...
    else if (action == SPELL)
    {
        ...
        if (1 || distance(attacker, make_pair(opBaseX, opBaseY)) < 8000)
        {
            ...
            {
                if (distance(attacker, m) < 2200 && m.health >= 10) sentCnt++;
            }//for m
        }//if
    }//else if
...
int main()
{
    cin >> myBaseX >> myBaseY; cin.ignore();
    ...
    while (1) {
        roundCnt++;
        cin >> myHealth >> myMana >> opHealth >> opMana; cin.ignore();
        ...
        sort(entities[monster].begin(), entities[monster].end(), [](auto a, auto b) { return a.distToMyBase < b.distToMyBase; });
        ...
        if (!shieldedMonster && (entities[monster].empty() || entities[monster][0].distToMyBase > 2000 && entities[monster][0].distToMyBase > entities[myHero][0].distToMyBase)) set_reaper(entities, 0, 1100, 7800);
        else set_defender(entities, 0);

        if ((!entities[monster].empty() && entities[monster][0].distToMyBase < 5000 && entities[myHero][1].distToMyBase < entities[myHero][0].distToMyBase && entities[myHero][1].distToMyBase < entities[myHero][2].distToMyBase) || shieldedMonster) set_defender(entities, 1);
        else set_reaper(entities, 1, 5400, 5500);
        ...
        if (roundCnt < 100 || entities[monster].empty() || myMana < 20) set_reaper(entities, 2, 7800, 1100, true);
        else set_attacker(entities, 2);
    }//while
}//main
